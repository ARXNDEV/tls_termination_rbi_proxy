...
type FuncRespHandler func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response

// FuncRespHandler.Handle(req,ctx) <=> FuncRespHandler(req,ctx)
func (f FuncRespHandler) Handle(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
	return f(resp, ctx)
}
...
func HandleString(f func(s string, ctx *goproxy.ProxyCtx) string) RespHandler {
...
func HandleStringReader(f func(r io.Reader, ctx *goproxy.ProxyCtx) io.Reader) RespHandler{
	return FuncRespHandler(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
		if ctx.Error != nil {
			return nil
		}
