...
func (c *CertificateCache) Put(serverName, serverAddr string, cert tls.Certificate, transport http.RoundTripper) {
	...
	if c.TTL <= 0 {
		// TTL was never initialized; left at 0 it makes every Get() a miss
		// (re-minting the cert on every connection) and wipes the cache on
		// every Put. Default to a safe value.
