...
func SSLBump(conn net.Conn, serverAddr, user, authUser string, r *http.Request, gpt *GSProxyPassthru, gsproxy *GSProxy) {
	...
	// customHandler := http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
	// 	// Add your custom header here
	// 	req.Header.Set("X-GoogApps-Allowed-Domains", "accops.com")

	// 	// ProxyHandler is assumed to be a type or function that handles forwarding requests
	// 	ProxyHandler{
	// 		TLS:         true,
	// 		connectPort: port,
	// 		user:        authUser,
	...
	// Print the loaded domains to verify
	fmt.Println("Allowed Domains:", config.AllowedDomains)

	// Create a custom handler that uses the loaded allowed domains
	customHandler := http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		// Join the allowed domains into a comma-separated string
		domains := strings.Join(config.AllowedDomains, ", ")

		// Print the loaded domains to verify
		fmt.Println("[FINAL] Allowed Domains:", domains)

		// Set the X-GoogApps-Allowed-Domains header
		//req.Header.Set("X-GoogApps-Allowed-Domains", domains)

		// Assuming ProxyHandler is a type that implements http.Handler
		ProxyHandler{
			TLS:         true,
			connectPort: port,
			user:        authUser,
